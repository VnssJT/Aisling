...
public class MemoryManager : MonoBehaviour
{
    ...
    public void AddFragment(MemoryIndex memoryID, int fragmentID){
        ...
        //Debug.Log("MEMORY MANAGER: AddFragment_memories found count: " + memoriesFound.Count);
        //Debug.Log("MEMORY MANAGER: AddFragment_memoryID: " + memoryID);
        memoriesFound[(int) memoryID] = true;
        //Debug.Log("MEMORY MANAGER: memory " + memoryID + " found");

        AddMemoryToInventory(memoryID);
    ...
    void AddMemoryToInventory(MemoryIndex memoryID){
        Debug.Log("MEMORY MANAGER: telling inventory to delete " + memoryID + " fragments");
        Inventory.instance.deleteFragments(memoryID);

        // Look for the memory in the memories list
        foreach(Memory memory in memories){
            if(memory.MemoryID == memoryID){
                Inventory.instance.Add(memory);
                return;
            }
        }
    }
