...
public class MemoryUI : MonoBehaviour
{
    ...
    public void OnPrevButtonPressed(){
        ...
        if(currentImageDisplayed > 0){
            ...
            float canvasWidth = ImagesParent.gameObject.GetComponent<RectTransform>().rect.width;
            float canvasXscale = ImagesParent.gameObject.GetComponent<RectTransform>().localScale.x;
            Debug.Log("MEMORYUI: Canvas width: " + canvasWidth);
            Debug.Log("MEMORYUI: Canvas size: " + canvasXscale);
            Vector3 newPos = transform.position - new Vector3((canvasWidth * canvasXscale * currentImageDisplayed), 0);
    ...
    public void OnNextButtonPressed(){
        if(currentImageDisplayed < ImagesParent.transform.childCount){
            currentImageDisplayed++;
            Vector3 newPos = transform.position - new Vector3((gameObject.GetComponent<RectTransform>().rect.width * currentImageDisplayed), 0);
            StartCoroutine(SmoothMove(ImagesParent.transform.position, newPos, easing));
        }
    }
