...
public class MemoryUI : MonoBehaviour
{
    ...
    public void displayImages(MemoryManager.MemoryIndex memoryID){
        ...
        int nImages = memImage.nImages;

        // For eevery image the memory holds
        for (int i = 0; i < nImages; i++)
        {
            // Create an instance
            var newImage = Instantiate(ImageHolderPrefab, Vector3.zero, Quaternion.identity, ImagesParent.transform);

            // Set the srpite
            newImage.GetComponent<Image>().sprite = (Sprite) Resources.Load("Images/" + imageRootName + memoryID.ToString() + i);
        }
